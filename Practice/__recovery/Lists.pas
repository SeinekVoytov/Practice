...
implementation
  ...
  Procedure TCarList.Sort(Comparator: TComparator; Brand: String; BrandsList: TBrandList);
    Var
      Prev, Current, TempHead, Temp, Dummy: PCar;
      TempList: TCarList;
      IsStarted: Boolean;
    Begin

        TempHead := Head;
        while (BrandsList.GetBrand(TempHead^.Next^.Code) <> Brand) do
          Begin
            TempHead := TempHead^.Next;
          End;
        ...
        while (Current <> nil) and (BrandsList.GetBrand(Current^.Code) = Brand) do
          Begin
